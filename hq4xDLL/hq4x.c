...
static int   RGBtoYUVtable[65536];
...
static const int RGBtoYUV(int c) {
	int r=(c&0xff0000)>>16>>3<<11;
	int g=(c&0xff00)>>8>>2<<5;
	int b=(c&0xff)>>3;
	return RGBtoYUVtable[r|g|b];
}

 void Interp1(unsigned char * pc, int c1, int c2)
{
  *((int*)pc) = (c1*3+c2) >> 2;
}

 void Interp2(unsigned char * pc, int c1, int c2, int c3)
{
  *((int*)pc) = (c1*2+c2+c3) >> 2;
}

 void Interp3(unsigned char * pc, int c1, int c2)
{
  //*((int*)pc) = (c1*7+c2)/8;
 ...
 void Interp5(unsigned char * pc, int c1, int c2)
{
  *((int*)pc) = (c1+c2) >> 1;
}

 void Interp6(unsigned char * pc, int c1, int c2, int c3)
{
 ...
 void Interp7(unsigned char * pc, int c1, int c2, int c3)
{
  //*((int*)pc) = (c1*6+c2+c3)/8;
 ...
 void Interp8(unsigned char * pc, int c1, int c2)
{
  //*((int*)pc) = (c1*5+c2*3)/8;
...
//    LUT16to32[i] = ((i & 0xF800) << 8) + ((i & 0x07E0) << 5) + ((i & 0x001F) << 3);
	  // classic99 15 bit
//	  LUT16to32[i] = ((i & 0xec00) << 9) + ((i & 0x03E0) << 6) + ((i & 0x001F) << 3);
  }
  ...
  for (k=0; k<32; k++)
  {
    ...
    u = 128 + ((r - b) >> 2);
    v = 128 + ((-r + 2*g -b)>>3);
    RGBtoYUVtable[ (i << 11) + (j << 5) + k ] = (Y<<16) + (u<<8) + v;
  }
}

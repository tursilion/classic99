...
INSTRUCTION(call_WORD)	 { PUSH(PC + 3); PC = READ_16(PC + 1);	++CallHits[PC];    return 17;}
...
CPU_Z80_API zusize z80_run(Z80 *object, zusize cycles)
	...
	while (CYCLES < cycles)
		...
		if (NMI)
            ...
            // treat this as one instruction so we see it in the log
            if (cycles > 900000) break;
        ...
        // flag to run only one instruction
        if (cycles > 900000) break;
		}

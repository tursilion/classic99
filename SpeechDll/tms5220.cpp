...
bool tms5220_device::data_write(int data)
{
	...
	if (m_DDIS) // If we're in speak external mode
	{
		...
		{
			// Tursi: what do you mean it should never happen? Speak external would allow it... maybe it's filtered
			// earlier in MAME...
...
void tms5220_device::device_start(speechrom_device *pRom)
{
#if 0
	if (m_speechrom_tag)
	{
...
void tms5220_device::device_reset()
{
	...
	m_old_zpar = false;
#endif
	// TURSI - fix these build warnings for sloppy type conversion
	m_current_energy =  m_previous_energy = 0;
	m_new_frame_energy_idx = (uint8_t)m_current_energy;
	m_current_pitch = 0;
	m_new_frame_pitch_idx = 0;

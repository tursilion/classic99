...
void tms5220_reset_chip(void *chip)
{
	...
	// this call is pointless - all it does is load a single 0 nibble into
	// the address shift register. We want to RESET the address, not offset it.
...
/**********************************************************************************************
     ...
     -return false if speak external fifo is full
	 - fRetry just mutes the full fifo debug if set
***********************************************************************************************/
...
tryagain:
    ...
    {
        if (tms->fifo_count <= FIFO_LOW )	// was 9
           goto empty;
...
static int parse_frame(struct tms5220 *tms, int the_first_frame)
{
	...
	if (indx == 0 || indx == 15)
	{
		...
		if (indx == 15)
		{
			logerror("Stop frame - done talking.\n");
			tms->fifo_head = tms->fifo_tail = tms->fifo_count = tms->fifo_bits_taken = 0;
